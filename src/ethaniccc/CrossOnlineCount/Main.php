                    ...
                    if (isset($server[0])) {
                        if ($server[0] === "server") {
                            if (empty($server[1])) {
                                ...
                                if (!$this->isValidIP($server[1]) && $this->is_valid_domain_name($server[1])) {
                                    $ip = "not_a_valid_ip";
                                } else {
                                    $ip = $server[1];
                                }
                            ...
                            if ($ip !== "not_a_valid_ip" && $port !== "invalid_port") {
                                $this->getServer()->getAsyncPool()->submitTask(
                                    new QueryServer(
                                        $ip, $port, $entity->getId(),
                                        $this->getConfig()->get("server_online_message"),
                                        $this->getConfig()->get("server_offline_message")
                                    )
                                );
                            }
